...
def plot_time_vs_cpus(data, stats_group, group_value, filename):
    ...
    if stats_group == 'nodes':
        # Plot the data with error bars
        plt.figure(figsize=(6, 4))
        suffix = f"gpus_for_{group_value}_nodes"
        plot_title = f'{data_name}: {group_value} Nodes with/without GPU offloading'
        ...
        for gpu_count in data['gpu'].unique():
            ...
            if gpu_count == 1:
                ...
                connector = "without"
            plt.errorbar(gpu_group['cpus'], gpu_group['mean'], yerr=gpu_group['std'], label=f'{connector} GPU', fmt='-o', capsize=5)
        ...
        # Plot the data with error bars
        plt.figure(figsize=(12, 4))
        suffix = f"nodes_for_{group_value}_gpus"
        if group_value == 1:
            connector = 'with'
        else:
            connector = 'without'
        plot_title = f'{data_name}: Nodes {connector} GPU offloading'

        for node_count in data['nodes'].unique():
            node_group = grouped[grouped['nodes'] == node_count]
            plt.errorbar(node_group['cpus'], node_group['mean'], yerr=node_group['std'], label=f'{node_count} Nodes', fmt='-o', capsize=5)
...
if __name__ == "__main__":
    ...
    for gpu_count in unique_gpus:
        ...
        plot_time_vs_cpus(data, 'gpu', gpu_count, output_file)  

    for node_count in unique_nodes:
        plot_time_vs_cpus(data, 'nodes', node_count, output_file)  
